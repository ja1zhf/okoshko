...
const Page = () => {
  ...
  return (
    <PageDiv>
      ...
      <StatusListDiv>
        <StatusButton $isSelected={selectedStatus === -1} onClick={() => setSelectedStatus(-1)}>Все</StatusButton>
        {statuses.map((item, index) => (
          <StatusButton key={index} $isSelected={selectedStatus === index} onClick={() => setSelectedStatus(index)}>{item}</StatusButton>
        ))}
      </StatusListDiv>
      <OrdersListDiv>
        {!isMasterOrders && userOrders.map((order, index) => {
